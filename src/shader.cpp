...
GLuint Shader::compile(std::string shaderSource, GLenum type) {
    ...
    // Check if there is a compile message to display
    if (result != GL_TRUE) {
        int length = 0;
        glGetShaderiv(id, GL_INFO_LOG_LENGTH, &length);
        char * message = new char[length];
        glGetShaderInfoLog(id, length, &length, message);
        std::cout << "Shader compile ERRÖÖR: " << message << std::endl;
        delete[] message;
        return 0;
    }
...
GLuint Shader::createShader(const char * vertexShaderFilename, const char * fragmentShaderFilename) {
    std::string vertexShaderSorce = parse(vertexShaderFilename);
    std::string fragmentShaderSource = parse(fragmentShaderFilename);

    GLuint program = glCreateProgram();
    GLuint vs = compile(vertexShaderSorce, GL_VERTEX_SHADER);
